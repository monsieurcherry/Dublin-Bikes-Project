...
def weather_forecast(days_from_today, hour):
    ...
    date_time = datetime.datetime(
        today.year, today.month, today.day + days_from_today, hour)
    ...
    index = 0
    closest_date = abs(forecast_info_json.get('list')[
                       0].get('dt') - time_unix_timestamp)
    for i in range(40):
        if abs(forecast_info_json.get('list')[i].get('dt') - time_unix_timestamp) < closest_date:
            closest_date = abs(forecast_info_json.get('list')[
                               i].get('dt') - time_unix_timestamp)
        index = i
    ...
    weather_data['description'] = weather_info.get('weather')[
        0].get('description')
#     # Convert the dictionary to a DataFrame
    future_weather_data = pd.DataFrame(weather_data, index=[0])
#     # One-hot encode the 'description' column
    future_weather_data = pd.get_dummies(
        future_weather_data, columns=['description'])
...
def predict_for_future_date(station_id, days_from_today, hour):
    ...
    is_weekday = int((future_date.weekday() >= 0) &
                     (future_date.weekday() <= 4))
    is_busy_hours = int(((hour >= 7) & (hour <= 10)) |
                        ((hour >= 16) & (hour <= 19)))
